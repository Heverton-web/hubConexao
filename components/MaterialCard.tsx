...
export const MaterialCard: React.FC<MaterialCardProps> = ({ material, onView }) => {
  ...
  return (
    <div className="group relative bg-surface/50 backdrop-blur-sm rounded-2xl border border-border overflow-hidden hover:shadow-2xl hover:shadow-accent/10 transition-all duration-300 hover:-translate-y-1 hover:border-accent/30">
      ...
      <div className="p-6 relative z-10">
        <div className="flex justify-between items-start mb-6">
          <div className="p-3 bg-page/80 rounded-xl border border-border shadow-sm group-hover:shadow-md transition-shadow">
            {getIcon()}
          </div>
          <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-page/50 rounded-lg text-muted border border-border/50">
            {getLabel()}
          </span>
        </div>
            ...
            {languages.map(lang => {
              ...
              return (
                <button
                  ...
                  disabled={!hasAsset}
                  className={`
                    px-2.5 py-1.5 text-xs rounded-lg border transition-all duration-300 flex items-center gap-1.5 font-medium
                    ${hasAsset 
                      ? 'border-accent/30 bg-accent/5 text-accent hover:bg-accent hover:text-white hover:border-accent hover:shadow-lg hover:shadow-accent/20 cursor-pointer transform hover:scale-105' 
                      : 'border-border/50 bg-page/30 text-muted/40 cursor-not-allowed grayscale'}
                  `}
                >
