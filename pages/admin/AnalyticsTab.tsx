...
const AnalyticsDetailModal = ({ material, logs, onClose, lang }: { material: Material, logs: AccessLog[], onClose: () => void, lang: Language }) => {
    return createPortal(
        <div className="fixed inset-0 flex items-center justify-center p-6 bg-page/80 backdrop-blur-xl animate-reveal" style={{ zIndex: 9999 }}>
            <div className="aura-glass rounded-[2rem] w-full max-w-3xl shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col max-h-[85vh] overflow-hidden relative">
                {/* Header Aura */}
                <div className="px-10 py-8 flex justify-between items-center bg-white/[0.01]">
                    ...
                    <button onClick={onClose} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/[0.03] text-white/20 hover:text-white hover:bg-white/10 transition-all outline-none">
                        <X size={24} />
                    </button>
                </div>
                                ...
                                {logs.map(log => (
                                    <tr key={log.id} className="group hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-10 py-6">
                                            <span className="px-3 py-1 rounded-lg bg-white/[0.03] text-[9px] font-black uppercase tracking-widest text-white/30">
                                                {log.userRole}
                                            </span>
                                        </td>
                                        <td className="px-10 py-6">
                                            <span className="px-3 py-1 rounded-lg bg-accent/5 text-[9px] font-black uppercase tracking-widest text-accent">
                                                {log.language}
                                            </span>
                                        </td>
    ...
    return (
        <div className="animate-reveal space-y-10 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="aura-glass p-8 rounded-[2rem] flex items-center gap-6 group hover:bg-white/[0.02] transition-all duration-500">
                    <div className="w-16 h-16 rounded-2xl bg-accent/5 flex items-center justify-center text-accent group-hover:scale-105 transition-transform">
                        <Eye size={28} />
                    </div>
                    <div>
                ...
                <div className="aura-glass p-8 rounded-[2rem] flex items-center gap-6 group hover:bg-white/[0.02] transition-all duration-500">
                    <div className="w-16 h-16 rounded-2xl bg-purple-500/5 flex items-center justify-center text-purple-400 group-hover:scale-105 transition-transform">
                        <Users size={28} />
                    </div>
                    <div>
                ...
                <div className="aura-glass p-8 rounded-[2rem] flex items-center gap-6 group hover:bg-white/[0.02] transition-all duration-500 overflow-hidden relative">
                    ...
                    <div className="w-16 h-16 rounded-2xl bg-success/5 flex items-center justify-center text-success group-hover:scale-105 transition-transform shrink-0">
                        <TrendingUp size={28} />
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 aura-glass p-8 rounded-[2.5rem]">
                    ...
                    <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData.trendData}>
                                ...
                                <XAxis dataKey="date" stroke="rgba(255,255,255,0.1)" tick={{ fontSize: 9, fontWeight: 900, fill: 'rgba(255,255,255,0.2)' }} tickLine={false} axisLine={false} dy={10} />
                                <YAxis stroke="rgba(255,255,255,0.1)" tick={{ fontSize: 9, fontWeight: 900, fill: 'rgba(255,255,255,0.2)' }} tickLine={false} axisLine={false} dx={-10} />
                                <Tooltip
                                    contentStyle={{ backgroundColor: 'rgba(12, 13, 17, 0.8)', border: 'none', borderRadius: '20px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}
                                    itemStyle={{ color: '#00D1FF', fontSize: '11px', fontWeight: '900', textTransform: 'uppercase' }}
                                    labelStyle={{ color: 'rgba(255,255,255,0.3)', fontSize: '9px', marginBottom: '6px', textTransform: 'uppercase', fontWeight: 'black', letterSpacing: '0.1em' }}
                                    cursor={{ stroke: 'rgba(0, 209, 255, 0.2)', strokeWidth: 2 }}
                                />
                                <Area type="monotone" dataKey="count" stroke="#00D1FF" strokeWidth={3} fillOpacity={1} fill="url(#colorCount)" animationDuration={2000} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                    ...
                    <div className="h-72 flex flex-col items-center">
                        <ResponsiveContainer width="100%" height="80%">
                            <PieChart>
                                ...
                                <Tooltip
                                    contentStyle={{ backgroundColor: '#0C0D11', border: 'none', borderRadius: '16px', backdropFilter: 'blur(10px)' }}
                                    ...
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-4 min-w-0">
                                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black transition-all
                                                ${index === 0 ? 'bg-yellow-500/10 text-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.1)]' :
                ...
                <div className="aura-glass rounded-[2rem] overflow-hidden">
                    ...
                    <div className="p-8 space-y-4">
                        {activeUsersRanking.map((user, index) => (
                            <div key={index} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] hover:bg-white/[0.04] transition-all group">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-accent/5 flex items-center justify-center text-accent font-black text-sm group-hover:scale-110 transition-transform shadow-[0_0_15px_rgba(0,209,255,0.05)]">
                                        {user.name.charAt(0)}
                                    </div>
                                    <div>
            ...
            {/* Metrics Table Aura */}
            <div className="aura-glass rounded-[2.5rem] overflow-hidden">
                <div className="px-10 py-8 flex justify-between items-center bg-white/[0.01]">
                                ...
                                {aggregatedMetrics.map(item => {
                                    ...
                                    return (
                                        <tr key={item.id} className="group hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="p-8">
                                                <span className="px-3 py-1 rounded-lg bg-white/[0.03] text-[9px] font-black uppercase tracking-widest text-white/30 group-hover:text-accent/60 transition-all">{mat.type}</span>
                                            </td>
                                            ...
                                            <td className="p-8 text-right">
                                                <button
                                                    ...
                                                    className="w-10 h-10 inline-flex items-center justify-center rounded-xl bg-white/[0.05] text-white/20 hover:text-accent hover:bg-accent/10 transition-all outline-none"
                                                >
                                                    <BarChart2 size={16} />
                                                </button>
                                            </td>
